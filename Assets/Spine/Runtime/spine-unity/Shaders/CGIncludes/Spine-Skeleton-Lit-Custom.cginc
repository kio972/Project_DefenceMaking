...
float3 GetEmission (VertexOutput i) {
		...
		//#if defined(_EMISSION_MAP)
			return tex2D(_EmissionTex, i.uv0.xy) * _Emission;
	//	#else
	//		return float3(0,0,0);//_Emission;
	//	#endif
	//#else
	//	return 0;
	#endif
...
VertexOutput vert (appdata v) {
	...
	float3 Normal = UnpackScaleNormal(tex2Dlod(_NormalTex, float4(TRANSFORM_TEX(o.uv0, _NormalTex),0.0,0.0)), _NormalBump);
	Normal = float3(0,0,dot(eyeNormal, Normal));
	Normal = Normal.xyz;
	// Lights
	half3 lcolor = half4(0,0,0,1).rgb + color.rgb * glstate_lightmodel_ambient.rgb;
	for (int il = 0; il < LIGHT_LOOP_LIMIT; ++il) {
		lcolor += computeOneLight(il, eyePos, Normal, color);
	}
